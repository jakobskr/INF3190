...
//TODO: Make this on signle int add shifting
struct mip_frame {
  uint8_t tra:3;
...
void forward_message(struct arp_cache* arp_current, struct route_backlog *route_current) {
  ...
  size_t mip_size = sizeof(struct mip_frame) + route_current->length;
  if (mip_size > 1500) {
  ...
  mip_ramme->tra = 4;
  mip_ramme->mip_dst = route_current->mip;
  if (route_current->from == 0) {mip_ramme->mip_src = arp_current->mip_src;}
  else {mip_ramme->mip_src = route_current->from;}
  mip_ramme->length = route_current->length / 4;
  mip_ramme->ttl = route_current->ttl;
  memcpy(mip_ramme->content, route_current->msg, route_current->length);
  ...
  if (debug) {
    printf("ETH_MSG OF %zd SENT: ", ret );
    printf("TRA: %d ", mip_ramme->tra);
    printf("SRC MIP: %d ",mip_ramme->mip_src);
    printf("LEN: %d ", (mip_ramme->length * 4));
    printf("DST MIP: %d ", mip_ramme->mip_dst);
    printf("TTL: %d", mip_ramme->ttl);
    printf("\n");
  }
...
void send_transport_message(struct arp_cache* arp_current, unsigned char* msg, int length, int ttl) {
  ...
  size_t mip_size = sizeof(struct mip_frame) + length;
  if (mip_size > 1500) {
    printf("MIP_PACKAGE EXCEEDS 1500 BYTE\nMESSAGE DISCARDED\n");
    return;
  }
  ...
  mip_ramme->length = length / 4;
  mip_ramme->ttl = ttl;
  memcpy(mip_ramme->content, msg, length);
  size_t msg_size = sizeof(struct ethernet_frame) + mip_size;
  eth_ramme = malloc (msg_size);
  memcpy(eth_ramme->destination, arp_current->dst_mac, 6);
  memcpy(eth_ramme->source, arp_current->src_mac, 6);
...
void send_unicast(struct arp_cache *arp_current,unsigned char *buf, int length) {
  ...
  mip_ramme->length = length / 4;
  mip_ramme->ttl = 15;
  memcpy(mip_ramme->content, buf, length);
  int msg_size = sizeof(struct ethernet_frame) + mip_size;
  eth_ramme = malloc(msg_size);
  memcpy(eth_ramme->destination, arp_current->dst_mac, 6);
  memcpy(eth_ramme->source, arp_current->src_mac, 6);
  eth_ramme->protocol = htons(ETH_P_MIP);
...
void send_broadcast(unsigned char *buf, int length) {
  ...
  while (arp_current != NULL) {
    struct mip_frame* mip_ramme;
    struct ethernet_frame* eth_ramme;
    int mip_size = sizeof(struct mip_frame) + length;
    mip_ramme = malloc(mip_size);
    mip_ramme->tra = 2;
    mip_ramme->mip_dst = 255;
    mip_ramme->mip_src = arp_current->mip_src;
    mip_ramme->length = length / 4;
    mip_ramme->ttl = 15;
    memcpy(mip_ramme->content, buf, length);
...
void handle_ethernet_socket(struct epoll_control *epctrl, struct fdInfo *info) {
  ...
  //ROUTING MESSAGE RECIEVED
    ...
    routing_info[0] = mip_ramme->mip_src;
    memcpy(routing_info + 1, mip_ramme->content, ret - (sizeof(struct ethernet_frame) + sizeof(struct mip_frame)));
    ssize_t ret2 = send(epctrl->connected_router, routing_info, ret - (sizeof(struct ethernet_frame) + sizeof(struct mip_frame) - 1), 0);
  ...
  //TRANSPORTING MESSAGE RECIEVED
    ...
    if (src_in_arp_cache(mip_ramme->mip_dst) != NULL) {
      ...
      unsigned char buf[BUF_SIZE] = {0};
      unsigned char mip_addr = mip_ramme->mip_src;
      buf[0] = mip_addr;
      memcpy(buf + 1, &mip_ramme->content, mip_ramme->length * 4);
      ssize_t ret = send(epctrl->connected_unix,buf,sizeof(mip_addr) + mip_ramme->length * 4, 0);
      ...
      printf("%d exd %d \n",mip_ramme->mip_dst, mip_ramme->mip_src);
      if (epctrl->connected_forward == 0) {
        if (debug) {
          printf("NO ROUTER CONNECTED, PACKAGE DROPPED\n");
        }
      ...
      unsigned char dst = mip_ramme->mip_dst;
      ssize_t ret3 = send(epctrl->connected_forward, &dst, sizeof(dst),0);
      ...
      struct route_backlog *msg = malloc(sizeof(struct route_backlog) + mip_ramme->length * 4);
      msg->mip = mip_ramme->mip_dst;
      msg->from = mip_ramme->mip_src;
      msg->length = mip_ramme->length * 4;
      msg->ttl = mip_ramme->ttl - 1;
      msg->msg_next = NULL;
      memcpy(msg->msg, mip_ramme->content, msg->length);
...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    printf(RED "usage: [-h] <socket_applictaion> <Socket_router> <socket_forwarding> [mip_addresses]\n" RESET );
    exit(0);
  }
